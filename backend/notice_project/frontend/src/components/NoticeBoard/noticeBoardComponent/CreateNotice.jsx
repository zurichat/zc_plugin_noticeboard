...
function CreateNotice() {
	...
	const onSubmitHandler = async (values) => {
		if (isChecked) {
			fetch("http://localhost:8000/api/v1/sendemail?sendemail={should_send}&org=614679ee1a5607b13c00bcb7");
		}
		...
		if (values.title === "" || setEditorState === "") {
			return setErrorMessage("Field cannot be empty"), setErrorTitle("Field cannot be empty");
		}
						...
						<form onSubmit={handleSubmit}>
							<Box className={classes.header}>
								...
								<Box display="flex" margin="0 auto" flexDirection="column" width="fit-content" justifyContent="center">
									<Hidden mdDown>
										<Button type="submit" variant="contained" className={classes.button} color="primary" disableRipple>
											{isSubmitting ? <CircularProgress className={classes.buttonSubmit} /> : "Publish Notice"}
										</Button>
										<br />
										<p>
											<input type="checkbox" checked={isChecked} onChange={(e) => setIsChecked((prevIsChecked) => !prevIsChecked)} />
							...
							<Box className={classes.recipient}>
								<Box width="100%" pt="30px" display="flex" flexDirection="column" className={classes.form}>
									<Box pb="10px">
